...
import { ethers, TypedDataDomain, TypedDataField, JsonRpcProvider } from 'ethers';
import { SmartAccount__factory } from 'typechain-types/factories/contracts/SmartAccount__factory';
...
import { blockchainConfig, logError } from './config';
...
export function getEntryPoint(): EntryPoint {
    try {
        ...
        return EntryPoint__factory.connect(blockchainConfig.entryPointAddress, provider as any);
    } catch (error) {
...
export class AccountAbstraction {
    ...
    async createUserOp({
        ...
        const accountContract = SmartAccount__factory.connect(sender, this.provider as any);
     ...
     */
    async executeUserOps(userOps: UserOperation[], beneficiary: string): Promise<ethers.TransactionResponse> {
        try {
            ...
            const connectedEntryPoint = this.entryPoint.connect(this.signer as any);
            return await connectedEntryPoint.handleOps(packedUserOps, beneficiary) as any;
        } catch (error) {
    ...
    verifyTransaction(receipt: ethers.TransactionReceipt, targetContract: ethers.BaseContract): void {
        ...
        try {
            ...
            throw new Error("Failed to parse UserOperationEvent log: " + (e instanceof Error ? e.message : String(e)));
        }
        ...
        if (!success) {
            ...
            try {
                ...
                throw new Error("Failed to parse UserOperationRevertReason log: " + (e instanceof Error ? e.message : String(e)));
            }
            ...
            if (decodedError) {
                throw new Error(
                    `UserOperation reverted with custom error: ${decodedError.name}, args: ${JSON.stringify(decodedError.args)}`
                );
            } else {
